...
import { localizeCategory, localizeProduct, localizeSubcategory } from "@/data/product-localization";
    ...
    items.map((product) => {
      ...
      <article className="catalog-card" key={product.slug}>
        ...
        <div className="catalog-copy">
          <small>{localizedProduct.category}</small>
          <h3>{localizedProduct.name}</h3>
          <p>{localizedProduct.longDescription}</p>
          <ul>
            <li><Check size={15} /> {localizedProduct.capacities}</li>
            <li><Check size={15} /> {localizedProduct.materials}</li>
          </ul>
          <a href={`/products/${product.slug}`} className="text-link">
            Ver detalles del producto <ArrowRight size={17} />
          </a>
        </div>
      </article>
      );
                ...
                return subcategories.length ? (
                  <div className={`product-sidebar-group${selectedCategory === category ? " open" : ""}`} key={category}>
                    ...
                    >
                      ...
                      <span className="product-sidebar-label">{localizeCategory(category)}</span>
                    </Link>
                      ...
                      {subcategories.map((subcategory) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="product-sidebar-label">{localizeSubcategory(subcategory)}</span>
                          </Link>
                        );
                      })}
                    </div>
            ...
            <div className="product-category-content">
              <section className="product-category-group">
                <div className="product-category-heading">
                  ...
                  <h3>{selectedSubcategory ? localizeSubcategory(selectedSubcategory) : selectedCategory ? localizeCategory(selectedCategory) : "Todos los productos"}</h3>
                </div>
