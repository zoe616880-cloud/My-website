...
import guardrailStyles from "@/components/GuardrailProductPage.module.css";
import { getProduct, products, type Product } from "@/data/products";
import { localizeCategory, localizeProduct } from "@/data/product-localization";
...
export async function generateMetadata({
  ...
  const sourceProduct = getProduct((await params).slug);
  if (!sourceProduct) return {};
  const product = localizeProduct(sourceProduct);
  return {
          ...
          <aside className="catalog-detail-sidebar">
            <div className="catalog-side-card">
              ...
              {categories.map((category) => (
                <a className={category === sourceProduct.category ? "active" : ""} href={`/products?category=${categoryId(category)}`} key={category}>
                  <PackageCheck size={14} /> {localizeCategory(category)}
                </a>
              ))}
            </div>
