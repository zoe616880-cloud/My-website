...
export function Header() {
  ...
  const isGuardrailProductPage = pathname === "/products/industrial-platform-scales-guardrail-platform-scale";
  const isImmersiveHeader = isHome || isGuardrailProductPage;
  const isAdmin = pathname.startsWith("/admin");
  const scrolled = scrollProgress > 0.98;
  const headerStyle = isImmersiveHeader
  ...
  useEffect(() => {
    if (!isImmersiveHeader) {
      setScrollProgress(0);
      return;
    }

    const updateProgress = () => {
      const start = isGuardrailProductPage ? 220 : 0;
      const distance = isGuardrailProductPage ? 180 : 160;
  ...
  }, [isImmersiveHeader, isGuardrailProductPage]);
  ...
  return (
    <header
      className={`site-header${isImmersiveHeader ? " site-header-home" : ""}${isGuardrailProductPage ? " site-header-guardrail" : ""}${scrolled ? " site-header-scrolled" : ""}${routeSettled ? " site-header-route-settled" : " site-header-route-transition"}`}
      style={headerStyle}
    ...
    >
      <div className="header-inner">
        ...
        <div className="nav-stage">
          <nav className="desktop-nav locale-fixed" aria-label="Основная навигация">
            <Link className={isActive("/") ? "current" : ""} href="/" onClick={() => setActiveMenu(null)}>Главная</Link>
            ...
            <Link className={isActive("/about") ? "current" : ""} href="/about" onClick={() => setActiveMenu(null)}>О компании</Link>
            <Link className={isActive("/request-a-quote") ? "current" : ""} href="/request-a-quote" onClick={() => setActiveMenu(null)}>Контакты</Link>
          </nav>

          {menu ? (
            <div
              className={`mega-menu mega-menu-${activeMenu} locale-fixed`}
        ...
        <div className="header-actions locale-fixed">
      ...
      {open ? (
        <nav className="mobile-nav locale-fixed" aria-label="Мобильная навигация">
          <Link href="/" onClick={() => setOpen(false)}>Главная</Link>
          <Link href="/products" onClick={() => setOpen(false)}>Продукция</Link>
          <Link href="/news" onClick={() => setOpen(false)}>Новости</Link>
          <Link href="/about" onClick={() => setOpen(false)}>О компании</Link>
          <Link href="/request-a-quote" onClick={() => setOpen(false)}>Контакты</Link>
