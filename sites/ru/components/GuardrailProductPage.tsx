...
export function GuardrailProductPage({ product, parentCategoryHref }: GuardrailProductPageProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      document.body.classList.remove("guardrail-product-page-active");
  ...
  return (
    <>
      <main className={styles.page}>
          ...
          <div className={styles.heroStage}>
            ...
            <div className={styles.videoCard}>
              ...
              <div className={styles.videoOverlay}>
                <button className={styles.videoPlayButton} type="button" onClick={() => setIsVideoOpen(true)}>
                  <Play size={15} fill="currentColor" /> Production Video
                </button>
                <strong>See the frame, guardrail and workshop process</strong>
              </div>
            </div>
          </div>
          <nav className={styles.pageTrail} aria-label="Breadcrumb">
            <Link href="/products">Product</Link>
            <span>/</span>
            <strong>Guardrail Platform Scale</strong>
          </nav>
        </section>
        ...
        {isVideoOpen ? (
          <div className={styles.videoModal} role="dialog" aria-modal="true" aria-label="Production video">
            <button
            ...
            <div className={styles.videoModalPanel}>
              <button className={styles.videoModalClose} type="button" aria-label="Close video" onClick={() => setIsVideoOpen(false)}>
                <X size={22} />
              </button>
              <video
                controls
